...
from config import MINI_CLASSES_PATH, RELATIONSHIPS
...
def extract_subgraph(file_path):
    ...
    # lookup_table['P373'] = 'commons_category'
...
# def get_subclasses(subjects):
#     results = []
#     for subject in tqdm(subjects):
#         results.extend(_get_subclasses(subject))
#     return results


# def _get_subclasses(subject):
...
#     query_1 = """SELECT ?item WHERE {?item wdt:P279 wd:""" + subject + """ .}"""
#     result_1 = get_results(query_1)
#     result.extend([clean(result['item']['value']) for result in result_1['results']['bindings']])


#     query_2 = """SELECT ?item WHERE {wd:""" + subject + """ wdt:P279 ?item.}"""
#     result_2 = get_results(query_2)
#     result.extend([clean(result['item']['value']) for result in result_2['results']['bindings']])
...
if __name__ == '__main__':
    ...
    extract_subgraph(args.filePath)